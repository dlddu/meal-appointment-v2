...
const slotSummaries: SlotSummary[] = [
  { slotKey: '2099-05-03#LUNCH', date: '2099-05-03', mealType: 'LUNCH', availableCount: 2, availabilityRatio: 0.75 },
  { slotKey: '2099-05-03#DINNER', date: '2099-05-03', mealType: 'DINNER', availableCount: 1, availabilityRatio: 0.55 },
  { slotKey: '2099-05-04#BREAKFAST', date: '2099-05-04', mealType: 'BREAKFAST', availableCount: 0, availabilityRatio: 0.2 }
];

const participants: AppointmentViewResponse['participants'] = [
  { participantId: 'p1', nickname: '알파', submittedAt: '2024-05-02T12:00:00.000Z', responses: ['2099-05-03#LUNCH'] },
  { participantId: 'p2', nickname: '베타', submittedAt: '2024-05-02T13:00:00.000Z', responses: ['2099-05-03#LUNCH', '2099-05-03#DINNER'] }
];
...
describe('slot formatter utilities', () => {
  ...
  it('groups slot summaries by date and sorts in ascending order', () => {
    ...
    expect(grouped[0]).toMatchObject({ date: '2099-05-03', summaries: [{ mealLabel: '점심' }, { mealLabel: '저녁' }] });
    expect(grouped[1].date).toBe('2099-05-04');
  });

  it('formats slot keys into readable labels', () => {
    expect(formatSlotKey('2099-05-03#LUNCH')).toContain('5월 3일 점심');
  });
  ...
  it('builds a participant matrix aligned with slot summaries', () => {
    ...
    expect(matrix[0]).toMatchObject({ slotKey: '2099-05-03#LUNCH', participants: ['알파', '베타'] });
    expect(matrix[2]).toMatchObject({ slotKey: '2099-05-04#BREAKFAST', participants: [] });
  });

  it('exposes Korean formatted dates and weekdays', () => {
    expect(getDisplayDate('2099-05-03')).toContain('5월');
    expect(getWeekdayLabel('2099-05-03')).not.toHaveLength(0);
  });
});
