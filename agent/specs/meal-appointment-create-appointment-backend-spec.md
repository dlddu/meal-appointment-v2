...
- 모든 오류 응답은 `requestId`를 포함하여 프런트엔드가 추적 가능하도록 한다.

## 9. 로깅 및 관측성
- Pino 로거를 사용하며, 요청 ID 미들웨어로 각 요청에 `requestId`를 부여한다.
...
## 10. 비기능 요구사항
- API는 IP 기준 분당 30회 호출 제한을 적용한다(Express-rate-limit 등 미들웨어 활용). 제한 초과 시 429 응답과 표준 오류 본문을 반환한다.
...
- **통합 테스트:**
  ...
  - 레이트 리밋 초과 시 429 응답을 반환하는지 검증한다.
