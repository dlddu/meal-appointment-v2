...
7. `shareUrl`은 `/appointments/{appointmentId}` 상대 경로를 기본으로 하되 `PUBLIC_BASE_URL`이 설정되면 절대 URL을 반환할 수 있어야 한다.
...
12. Prisma 모델은 `summary`를 선택 필드로 정의하고 생성 시 `summary ?? ''` 처리를 적용해야 한다.
...
17. 레이트 리밋은 IP 기준 분당 30회로 설정하고 CORS는 허용 도메인을 명시해야 한다.
